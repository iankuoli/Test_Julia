...
include("models/BPR_model.jl")
include("models/BPR_new.jl")
include("models/BPR_learn.jl")
...
DATA = Dataset(6)
data_type, MaxItr, Ks, topK, lr, lambda = ExpSetting_BPR(DATA)
...
for kk = 1:length(Ks)
    for num = 1:NUM_RUNS
        ...
        # --------------------
        X = newBPR(K, M, N, ini_scale, usr_zeros, itm_zeros)
        ...
        while IS_CONVERGE == false
            ...
            # Train model
            Learn_BPR(X, lr, lambda, usr_idx, is_X_train, js_X_train, vs_X_train)
