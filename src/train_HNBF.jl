...
include("models/HNBF_model.jl")
include("models/HNBF_new.jl")
include("models/HNBF_learn.jl")
...
DATA = Dataset(6)
data_type, MaxItr, Ks, topK, prior = ExpSetting_HNBF(DATA)
...
for kk = 1:length(Ks)
    for num = 1:NUM_RUNS
        ...
        train_poisson = zeros(Int(ceil(MaxItr/likelihood_step)), 2)
        test_poisson = zeros(Int(ceil(MaxItr/likelihood_step)), 2)
        valid_poisson = zeros(Int(ceil(MaxItr/likelihood_step)), 2)
        ...
        # --------------------
        X = newHNBF(K, M, N, ini_scale, usr_zeros, itm_zeros, matX_train, prior)
        ...
        while IS_CONVERGE == false
            ...
            # Train model
            Learn_FastHNBF(X, is_X_train, js_X_train, vs_X_train)
            ...
            if likelihood_step > 0 && itr % likelihood_step == 0
                ...
                tmpXX = X.vec_matD_ui .* tmpX
                vstrain_poisson = Evaluate_LogLikelihood_Poisson(vs_X_train, tmpX)
                vstrain_nebinomoial = Evaluate_LogLikelihood_Poisson(vs_X_train, tmpXX)

                tmpX = sum(X.matTheta[is_X_test,:] .* X.matBeta[js_X_test,:], dims=2)[:]
                tmpXX = sum(X.matGamma[is_X_test,:] .* X.matDelta[js_X_test,:], dims=2)[:] .* tmpX
                vstest_poisson = Evaluate_LogLikelihood_Poisson(vs_X_test, tmpX)
                vstest_nebinomoial = Evaluate_LogLikelihood_Poisson(vs_X_test, tmpXX)

                tmpX = sum(X.matTheta[is_X_valid,:] .* X.matBeta[js_X_valid,:], dims=2)[:]
                tmpXX = sum(X.matGamma[is_X_valid,:] .* X.matDelta[js_X_valid,:], dims=2)[:] .* tmpX
                vsvalid_poisson = Evaluate_LogLikelihood_Poisson(vs_X_valid, tmpX)
                vsvalid_nebinomoial = Evaluate_LogLikelihood_Poisson(vs_X_valid, tmpXX)

                @printf("Train Loglikelihood of Poisson: %0.4f\n", Statistics.mean(vstrain_poisson))
                @printf("Train Loglikelihood of NB     : %0.4f\n", Statistics.mean(vstrain_nebinomoial))
                @printf("Valid Loglikelihood of Poisson: %0.4f\n", Statistics.mean(vsvalid_poisson))
                @printf("Valid Loglikelihood of NB     : %0.4f\n", Statistics.mean(vsvalid_nebinomoial))
                @printf(" Test Loglikelihood of Poisson: %0.4f\n", Statistics.mean(vstest_poisson))
                @printf(" Test Loglikelihood of NB     : %0.4f\n", Statistics.mean(vstest_nebinomoial))

                l_step_indx = Int(itr / likelihood_step)
                train_poisson[l_step_indx, 1] = Statistics.mean(vstrain_poisson)
                train_poisson[l_step_indx, 2] = Statistics.mean(vstrain_nebinomoial)
                test_poisson[l_step_indx, 1] = Statistics.mean(vstest_poisson)
                test_poisson[l_step_indx, 2] = Statistics.mean(vstest_nebinomoial)
                valid_poisson[l_step_indx, 1] = Statistics.mean(vsvalid_poisson)
                valid_poisson[l_step_indx, 2] = Statistics.mean(vsvalid_nebinomoial)
            end
