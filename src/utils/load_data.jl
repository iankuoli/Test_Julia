...
function LoadUtilities(train_path::AbstractString, test_path::AbstractString, valid_path::AbstractString)
    ...
    matIndex_train = DelimitedFiles.readdlm(train_path, ',');
    end

    if !isempty(test_path)
    matIndex_test = DelimitedFiles.readdlm(test_path, ',');
    end

    if !isempty(valid_path)
    matIndex_valid = DelimitedFiles.readdlm(valid_path, ',');
    end
    ...
    matX_train = sparse(convert(Array{Int,1}, matIndex_train[:,1]), convert(Array{Int,1}, matIndex_train[:,2]), matIndex_train[:,3], M, N)
    matX_test = sparse(convert(Array{Int,1}, matIndex_test[:,1]), convert(Array{Int,1}, matIndex_test[:,2]), matIndex_test[:,3], M, N)
    matX_valid = sparse(convert(Array{Int,1}, matIndex_valid[:,1]), convert(Array{Int,1}, matIndex_valid[:,2]), matIndex_valid[:,3], M, N)

    return matX_train, matX_test, matX_valid, convert(Int, M), convert(Int, N)
end
